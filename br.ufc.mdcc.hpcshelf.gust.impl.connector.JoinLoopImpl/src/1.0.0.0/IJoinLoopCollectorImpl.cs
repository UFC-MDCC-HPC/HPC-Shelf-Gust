...
namespace br.ufc.mdcc.hpcshelf.gust.impl.connector.JoinLoopImpl {
	public class IJoinLoopCollectorImpl<M0,TF,IKey,IValue,OKey,OValue,BF,GIF> : BaseIJoinLoopCollectorImpl<M0,TF,IKey,IValue,OKey,OValue,BF,GIF>, IJoinLoopCollector<M0,TF,IKey,IValue,OKey,OValue,BF,GIF>
		...
		private void clear_gif_set_PartitionTABLE () {
			...
			Bin_function_iterate.PartitionTABLE = Bin_function_iterate_gif.PartitionTABLE; //sync_perform.wait ();
		}
		...
		private void send_startup () {
			...
			foreach (int i in this.FacetIndexes[FACET_REDUCE]) {   
				...
				for (int k = 0, j = nr0; j < m_size; k++, j++)
					unit_ref [j] = new Tuple<int,int> (i/*,0 index of FACET_REDUCE*/, k);
			}
			...
			Task_binding_split_next.invoke (ITaskPortAdvance.CHUNK_READY);	//sync_perform.wait ();
		}
		...
		private void send_to_mappers () {
			...
			while (!end_computation) { // new iteration
				...
				thread_send_output.Join (); //sync_perform.wait ();
			}
